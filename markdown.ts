...
/**
 * Gets the list of system contracts for a given deployment.
 *
 * @param deployment Folder where the deployment is located.
 * @returns List of system contracts for the given deployment.
 */
const getPreDeploys = (deployment: string): ContractInfo[] => {
...
const main = async () => {
  for (const deployment of PUBLIC_DEPLOYMENTS) {
    ...
    md = addline(md, `## Deployments Contracts`)
    md = addline(
      md,
      buildContractsTable(
    ...
    md = addline(md, `## System Contracts`)
    md = addline(
      md,
      buildContractsTable(
        getPreDeploys(deployment.folder),
